...
const numsPerGroup = Math.ceil(3);
// Create array based on number of groups
const groupRucksacks = new Array(rucksacks.length/3)
  .fill('')
  .map((_, i) => rucksacks.slice(i * numsPerGroup, (i + 1) * numsPerGroup));
const badgeRucksack = [];
groupRucksacks.forEach((currentTeam) => {
    const comp1 = currentTeam[0];
    const comp2 = currentTeam[1];
    const comp3 = currentTeam[2];
    let commonChar = '';  
    for (let i = 0; i < comp3.length; i++) {
        const item = comp3[i];
        if(comp1.includes(item) && comp2.includes(item)) {
            commonChar = item;
            break;
        }
    }
    if(commonChar.toLowerCase() === commonChar) {
        badgeRucksack.push(commonChar.charCodeAt() - 96); //a starts at 97
    } else {
        badgeRucksack.push(commonChar.charCodeAt() - 64 + 26); //A starts at 65
    }
});
const sum = badgeRucksack.reduce((accumulator, currentVal) => accumulator + currentVal, 0);
console.log(sum);